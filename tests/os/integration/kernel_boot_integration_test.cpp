...
constexpr cpu::SQWORD64 TEST_MEMORY_BASE_REGISTER_VALUE = static_cast<cpu::SQWORD64>(TEST_MEMORY_BASE_ADDRESS);
constexpr cpu::SQWORD64 TEST_MEMORY_DISPLACEMENT = cpu::SQWORD64{8};
constexpr cpu::ADDRESS64 TEST_STORED_ADDRESS =
    TEST_MEMORY_BASE_ADDRESS + static_cast<cpu::ADDRESS64>(TEST_MEMORY_DISPLACEMENT);
constexpr cpu::SQWORD64 TEST_OS_THREAD_VALUE = cpu::SQWORD64{0x4D4E4F53};
...
TEST(KernelBootIntegrationTest, BootedKernelRunsAThreadContextThroughCpuExecutor)
{
    ...
    EXPECT_THAT(machine.physical_memory().read_qword(TEST_STORED_ADDRESS), Eq(static_cast<cpu::UQWORD64>(TEST_OS_THREAD_VALUE)));
    EXPECT_THAT(thread.cpu_state().registers().read(cpu::RegisterId::RBX), Eq(static_cast<cpu::UQWORD64>(TEST_OS_THREAD_VALUE)));
}
