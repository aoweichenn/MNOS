...
include/mnos/
  ...
  host/                 宿主机交互 adapter，例如 TerminalRunner、terminal backend、debugger session
  cpu/
  ...
  os/
    ...
    proc/               ProcessId、Process、process_context、UserProgram/UserLoader/UserProgramArguments、Elf64Loader、CopyOnWriteManager、FutexTable
...
src/
  host/                 mnos_console/mnos_gui 入口、终端 runner、debugger session 和宿主机 terminal backend 实现
```
...
Stage 15D / 15E-A / 15E-B / 15F-A / 15F-B / 15F-C / 15F-D 当前语义：
...
HostDebuggerSession       窗口 view-model，负责 terminal screen、状态、内存、CPU、registers、paging、cursor、run-control 和 trace summary
...
CPU snapshot              shell thread id/state、core、CPL、RIP/RSP/RFLAGS 和 paging CR3/CR2/PCID/generation
Register drill-down       独立 GPR 面板，显示 RAX/RBX/RCX/RDX/RSI/RDI/RBP/RSP/R8..R15
Paging drill-down         独立页表面板，显示 address-space root/arena、active paging state、RIP/RSP 的 PML4/PDPT/PD/PT index 和只读 entry walk
Instruction trace input   HostDebuggerSession 可消费 CPU ExecutionTrace，bounded FIFO 格式化 cycle/rip/opcode/halt/trap；Stage 16A 后 GUI Exec 可直接把用户程序 retired instruction trace 接入该面板
...
TerminalInputMode         AUTO/LINE/RAW，pipe 默认 line，交互 TTY 默认 raw
StreamTerminalBackend     当前 stdio backend，支持 getline line 模式和逐字节 raw-key 模式
Raw key decoder           解析 Enter、Backspace/Delete、Tab、Ctrl-C、Ctrl-D 和 ANSI CSI 方向键
...
mnos_gui                  macOS 原生窗口入口，左侧终端屏幕，右侧状态 + Registers/Paging/Actions/Instructions tabs，支持 raw-key focus、Send/Reset/Step/Exec/Run/Pause/Exit
GUI smoke                 mnos_gui --smoke 无窗口验证 HostDebuggerSession、shell 命令和 Exec instruction trace 链路
Host tests                覆盖 session boot/reset/snapshot、debugger frame、trace/run-control、register/paging drill-down、CPU ExecutionTrace 接入、输入事件、raw key、渲染 drain、clear/backspace、EOF、I/O error 和 replay 回归
```

Stage 16A 当前语义：

```text
Process lifecycle      Process 显式记录 parent_id、RUNNING/EXITED/REAPED 和 exit_code
EXIT syscall           有进程上下文时进入 Kernel::exit_process，关闭该进程全部 alive 线程并唤醒 waiters；旧无进程教学路径仍只结束当前线程
WAIT syscall           父进程按 child pid 等待：已退出则返回 exit_code 并 REAP，运行中则阻塞 waiter，无子进程返回 -ENOENT
exec_user_program      Kernel 使用 UserLoader 加载 UserProgram，按 byte ExecutableImage 逐条执行，处理 SYSCALL/#PF，并返回 UserProcessRunResult
...
Stage 7 仍保持教学范围：当前是确定性 APIC/timer/IPI/shootdown 和 scheduler handoff 入口，不是假装已经有完整 SMP kernel。Stage 8 已经把 cache/pipeline/perf counter 接上，Stage 9 已经补齐 per-core scheduler 当前底座，Stage 10-18 已经把用户进程、终端交互、文件系统、宿主机交互入口、GUI/CLI 共享 session、窗口 debugger、register/paging drill-down、CPU ExecutionTrace 面板入口、exec/wait/process lifecycle、VFS ELF64 exec 和用户程序初始栈 ABI 接入主线，下一步再逐步扩成 pipe/redirect、page cache/mmap 和更复杂的设备模型。
...
Stage 15E-A 已完成: host terminal backend + input event model + renderer boundary
...
Stage 15F-D 已完成: register/page-table drill-down / CPU ExecutionTrace instruction panel
Stage 16A 已完成: exec/wait/process lifecycle + GUI Exec instruction trace
Stage 17 已完成: ELF64 文件加载 + VFS run/exec 通道
Stage 18 已完成: 用户程序 argc/argv/envp 初始栈 ABI + shell run args
Stage 19 下一步: pipe/dup/redirect
Stage 20 后续: page cache / mmap / demand file paging / block DMA / async I/O
```

### Stage 19: shell 组合能力

```text
pipe syscall / pipe fd
dup/dup2
shell redirect
shell pipeline
```

### Stage 20: page cache、mmap、demand file paging
