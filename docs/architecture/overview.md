...
指令      MOV/MOVSX/MOVZX/LEA/ADD/SUB/CMP/INC/DEC/AND/OR/XOR/TEST/CMPXCHG/XADD/MFENCE/PUSH/POP/CALL/RET/JMP/Jcc/SETcc/CMOVcc/INT/SYSCALL/SYSRET/IRET/HLT
内存      小端 PhysicalMemory + MemoryBus + PagingState + page table + TLB + MMU + X86TsoMemoryModel
decode    ExecutableImage + REX.W + LOCK + ModRM/SIB + rel8/rel32 + RIP-relative + r/m8/r/m16/r/m32/r/m64 source widths
系统表    CoreId/CoreTopology + PrivilegeLevel + GDT + TSS + IDT + TrapFrame + TrapController
执行      Executor::step/run，可选 ExecutionTrace，对象 Program 和 byte image 共用语义，原子指令仍保持 direct switch
```
...
include/mnos/
  ...
  cpu/
    ...
    memory/             PhysicalMemory、MemoryBus、PagingState、PageTableEntry、PageTableBuilder、TLB、MMU、X86TsoMemoryModel
    system/             CoreId、CoreTopology、CPL、GDT、TSS、IDT、TrapFrame、TrapController
...
CpuState             保存当前 core id，后续 APIC/IPI/scheduler 可把线程绑定到具体核心
...
后续仍不应把整个 x86-64 ISA 一次性塞进 decoder。Stage 6 已经把原子操作、core topology 和 x86 TSO 教学模型接入主线；下一步应沿 APIC/timer、抢占式调度、IPI 和 TLB shootdown 推进，每个新增硬件行为都要有 unit/integration/benchmark/docs 支撑。
...
Stage 5 仍不假装已经完成完整现代内核：真实 timer tick、抢占式 context switch、用户态 ELF loader、COW fork、wait/sleep 队列、信号、APIC/IOAPIC、PCID/INVLPG/TLB shootdown 都留给 Stage 7+。多核同步的第一层语义已在 Stage 6 落地。
...
### Stage 7: timer、APIC、抢占、TLB shootdown
