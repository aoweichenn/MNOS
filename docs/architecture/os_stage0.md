...
Stage 15D / 15E-A / 15E-B / 15F-A / 15F-B / 15F-C 已经完成第一版宿主机交互终端和窗口 debugger 边界：
...
HostDebuggerSession   窗口 view-model，格式化 terminal screen、状态、内存、CPU、cursor、run-control 和 trace 信息
...
mnos_gui              macOS 原生窗口入口：builds/debug/mnos_gui，左侧终端屏幕，右侧状态/trace 面板，支持 raw-key focus、Send/Reset/Step/Run/Pause/Exit
GUI smoke             builds/debug/mnos_gui --smoke 可无窗口验证 HostDebuggerSession 链路
Test harness          覆盖 session boot/reset/snapshot、debugger frame、trace/run-control、输入事件、raw key、渲染 drain、clear/backspace、EOF、I/O error 和 replay 回归
```
...
```text
1. GUI debugger drill-down：寄存器/页表细分视图和真实指令 trace
