...
3. 常量使用有领域含义的全大写名称。
4. 不保留死代码、旧 ISA 分支、不可达 fallback。
5. 优先使用 `enum class`、`std::span`、`std::string_view`、RAII、强类型 value object、表驱动映射。
6. 避免无意义拷贝，热路径参数按引用或轻量值传递。
7. 避免运行时递归，树/图/队列类处理使用显式 stack、queue、worklist。
